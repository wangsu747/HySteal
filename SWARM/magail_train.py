...
from SWARM.utils.config_util import config_loader
from SWARM.utils.il_csv_adapter import normalize_il_dataframe, infer_obs_cols
from SWARM.magail_env import MAGAILLearnedEnv
from SWARM.bc_pre import bc_pretrain
...
@click.option("--bc_init", type=str, default=None, help="(optional) path to BC ckpt")
@click.option("--expert_csv", type=str, default="", help="override config.general.expert_csv_path")
@click.option("--dynamics_ckpt", type=str, required=True, help="trained offline dynamics checkpoint")
@click.option("--reset_from_t0_only", type=bool, default=True, help="sample learned-env resets only from t=0 expert states")
@click.option("--reset_noise_sigma", type=float, default=0.0, help="optional Gaussian noise on learned-env reset state")
...
@click.option("--bc_ckpt_path", type=str, default=None, help="where to save BC ckpt")
@click.option("--skip_bc_pretrain", is_flag=True, help="skip BC pretrain stage")
@click.option("--resume_jointpolicy_path", type=str, default="", help="resume JointPolicy weights from a saved *_JointPolicy.pt")
@click.option("--resume_value_path", type=str, default="", help="resume Value weights from a saved *_Value.pt")
@click.option("--resume_td_path", type=str, default="", help="resume discriminator weights from a saved *_TransformerDiscriminator.pt")
def main(train_mode, config_train, config_eval, num_agent, trial,
         ...
         expert_csv, dynamics_ckpt, reset_from_t0_only, reset_noise_sigma,
         bc_pretrain_epochs, bc_batch_size, bc_lr, bc_ckpt_path, skip_bc_pretrain,
         resume_jointpolicy_path, resume_value_path, resume_td_path):
    def _infer_layout_from_expert_csv(csv_path: str, n_agents_cli: int):
        ...
        for a in df_norm["agent"].tolist():
            if a not in sset:
                sset.add(a)
                seen.append(str(a))
        if n_agents_cli > 0 and len(seen) >= n_agents_cli:
    ...
    def _find_generator_policy(mail):
        for name in ["P", "policy", "generator", "pi", "actor", "policy_net", "G", "gen"]:
        ...
        raise AttributeError(f"[BC] Cannot find generator policy inside MAGAIL instance. Visible keys: {visible}")

    if train_mode:
        cfg_path = config_train
        exp_name = f"MILD2LearnedEnv_Train_{time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime())}_trial{trial}"
    else:
        cfg_path = config_eval
        exp_name = f"MILD2LearnedEnv_Eval_{time.strftime('%Y-%m-%d_%H:%M:%S', time.localtime())}_trial{trial}"
    ...
    print(f"dynamics_ckpt: {dynamics_ckpt}")
    ...
    print(f"reset_from_t0_only: {reset_from_t0_only}")
    print(f"reset_noise_sigma: {reset_noise_sigma}")
    print("=" * 80)
    ...
    mail = MAGAILLearnedEnv(
        config=config,
        log_dir=log_dir,
        exp_name=exp_name,
        num_agent=num_agent,
        dynamics_ckpt_path=dynamics_ckpt,
        init_csv_path=config["general"]["expert_csv_path"],
        reset_from_t0_only=reset_from_t0_only,
        reset_noise_sigma=reset_noise_sigma,
        bc_init=bc_init,
    )

    # Optional warm resume from previously saved learned-env MAGAIL checkpoints.
    if isinstance(resume_jointpolicy_path, str) and resume_jointpolicy_path.strip():
        jp = torch.load(resume_jointpolicy_path.strip(), map_location="cpu")
        if hasattr(jp, "state_dict"):
            mail.P.load_state_dict(jp.state_dict(), strict=True)
            print(f"[resume] loaded JointPolicy -> {resume_jointpolicy_path}")
        else:
            raise TypeError(f"resume_jointpolicy_path is not a saved JointPolicy object: {resume_jointpolicy_path}")

    if isinstance(resume_value_path, str) and resume_value_path.strip():
        v_loaded = torch.load(resume_value_path.strip(), map_location="cpu")
        if isinstance(v_loaded, (list, tuple)) and len(v_loaded) == len(mail.V):
            for i, v in enumerate(v_loaded):
                mail.V[i].load_state_dict(v.state_dict(), strict=True)
            print(f"[resume] loaded Value list -> {resume_value_path}")
        else:
            raise TypeError(
                f"resume_value_path must be a saved list/tuple of Value modules with len={len(mail.V)}: {resume_value_path}"
            )

    if isinstance(resume_td_path, str) and resume_td_path.strip():
        td_loaded = torch.load(resume_td_path.strip(), map_location="cpu")
        if hasattr(td_loaded, "state_dict"):
            mail.TD.load_state_dict(td_loaded.state_dict(), strict=True)
            print(f"[resume] loaded TransformerDiscriminator -> {resume_td_path}")
        else:
            raise TypeError(f"resume_td_path is not a saved discriminator object: {resume_td_path}")
    ...
    if (not skip_bc_pretrain) and (bc_pretrain_epochs is not None) and (bc_pretrain_epochs > 0):
        expert_csv_path = config["general"]["expert_csv_path"]
        ...
        print(f"[BC] expert_csv_path: {expert_csv_path}")
